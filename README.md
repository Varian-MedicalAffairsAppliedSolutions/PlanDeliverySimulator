...
3. Select complete **Varian trajectory `.bin` logs or pylinac `.csv` exports** from the same machine, energy and mode, review the detected energy and nominal log dose rate, and analyze using **Local axis response (~1 s windows)**. Automatic log calibration supports single-layer 80- or 120-leaf MLCs and RDS dual-layer BIN logs (28 + 29 leaf pairs). BIN metadata automatically fills energy and maximum dose rate: 600 MU/min for conventional X energies, 800 for RDS FFF, and 1400/2400 for TDS 6FFF/10FFF. Unknown metadata requires manual review. RDS profiles model both physical layers and planned zero-MU transitions, and cannot be applied to TDS plans. New RDS fits include acceleration into and out of beam-off sections; comparison plots show the predicted ramps. Re-analyze older RDS profiles to use this update.
