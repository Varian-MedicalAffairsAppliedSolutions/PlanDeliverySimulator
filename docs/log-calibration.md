# Legacy delivery-time calibration
...
The reader skips patient/plan metadata and beam names. Numeric subbeam headers are retained, but both formats use the existing delivery-window rules below so format choice does not alter the calibration target. All snapshots are retained for elapsed timing, including beam-off samples. Missing payload, invalid axes, unknown versions/scales, truncated flags and CP/MU resets are rejected. A trailing two-byte CRC is skipped, not verified; this is layout/value validation, not checksum verification.
...
One leaf-response parameter is fitted over a fixed 20–180 mm/s² search range in increments of 1. The gantry/collimator acceleration settings and all speed caps remain fixed. The fitted leaf parameter is an empirical controller-response proxy, not a measured motor acceleration or proof of the machine's true limit. Manual startup and per-CP overheads are replaced in this mode, rather than added again; their saved values remain relevant to the Legacy comparison baseline.
...
Both the standalone page and modal provide eight charts: CP progress, cumulative MU, unwrapped gantry/collimator angles, gantry/collimator speeds, fastest-leaf speed, and dose rate. Select a log/arc, set a centered moving-average window (default 0.4 s), hide traces with the legend, or Ctrl-wheel/pinch to zoom. Reset zoom restores all charts.
...
Automatic reconstruction currently supports 80- and 120-leaf single-layer MLC logs, not RDS dual-layer logs. Enter the nominal dose rate used for the logged deliveries; the CSV does not establish the planned dose-rate setting. Select profiles for the correct machine, energy and mode yourself; the profile name is descriptive, not an automatic machine identifier.

The parser uses named columns, recorded sampling intervals, actual beam-hold state and machine-to-IEC angle conversion. Actual CP progression locates the commanded control points. A single-subbeam log includes internal holds within its elapsed delivery window. Multiple-subbeam logs require an unambiguous one-to-one mapping of delivery windows to declared subbeams; ambiguous holds, truncated data and CP/MU resets are rejected. Time outside delivery windows is reported separately, not folded into the correction.
