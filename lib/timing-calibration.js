...
(function(root) {
    ...
    function mlcGeometry(model,leafCount) {
        if(model===6){
            if(leafCount!==114)throw new Error('RDS model 6 requires 114 leaves after its two carriage channels.');
            return [{type:'MLCX1',boundaries:Array.from({length:29},(_,i)=>-140+i*10)},
                {type:'MLCX2',boundaries:Array.from({length:30},(_,i)=>-145+i*10)}];
        }
        if(leafCount!==120 || ![2,3].includes(model))throw new Error('Unsupported MLC geometry for visual playback.');
        const hd=model===3,outer=hd?14:10,inner=hd?32:40;
        const widths=[...Array(outer).fill(hd?5:10),...Array(inner).fill(hd?2.5:5),...Array(outer).fill(hd?5:10)];
        const boundaries=[-widths.reduce((a,b)=>a+b,0)/2];widths.forEach(w=>boundaries.push(boundaries.at(-1)+w));
        return [{type:'MLCX',boundaries}];
    }
    function mlcLayers(leaves,model,axisScale) {
        const native=axisScale!==2,left=values=>values.map(v=>native?-v:v);
        if(model===6){
            if(leaves.length!==114)throw new Error('Invalid RDS leaf layout.');
            // After carriages: X2 28-leaf bank, X2 29-leaf bank,
            // X1 28-leaf bank, X1 29-leaf bank (TrajectoryLog.NET SX2 layout).
            return [{type:'MLCX1',positions:[...left(leaves.slice(57,85)),...leaves.slice(0,28)]},
                {type:'MLCX2',positions:[...left(leaves.slice(85,114)),...leaves.slice(28,57)]}];
        }
        const half=leaves.length/2;
        return [{type:'MLCX',positions:[...left(leaves.slice(half)),...leaves.slice(0,half)]}];
    }
    function inspectBinHeader(input) {
        ...
        if(!bytes || bytes.length<1024)throw new Error('Incomplete trajectory BIN header.');
        const view=new DataView(bytes.buffer,bytes.byteOffset,bytes.byteLength);
        const text=(a,b)=>String.fromCharCode(...bytes.subarray(a,b));
        if(text(0,16).split('\0')[0]!=='VOSTL')throw new Error('Not a Varian trajectory BIN (VOSTL signature missing).');
        const version=text(16,32).split('\0')[0].trim(),count=view.getInt32(40,true),tail=44+8*count;
        if(view.getInt32(32,true)!==1024 || count<1 || tail+20>1024)throw new Error('Invalid trajectory BIN header layout.');
        const mlcModel=view.getInt32(tail+16,true),machine=mlcModel===6?'RDS':[2,3].includes(mlcModel)?'TDS':null;
        // Read only the energy line; never retain patient/plan identifiers.
        const raw=Number(version)>=4?text(tail+20,1024).split(/[\r\n\0]+/).find(line=>/^Energy\s*:/i.test(line)):null;
        const match=raw?.split(':').slice(1).join(':').trim().match(/^(\d+(?:\.\d+)?)\s*(X)?\s*(FFF)?$/i);
        const energy=match&&(match[2]||match[3])?`${Number(match[1])}X${match[3]?' FFF':''}`:null;
        const nominalDoseRate=!energy?null:!match[3]?600:machine==='RDS'?800:machine==='TDS'?({6:1400,10:2400}[Number(match[1])]??null):null;
        return {version,mlcModel,machine,energy,nominalDoseRate};
    }
    async function inspectFiles(files) {
        const rows=await Promise.all(files.map(async file=>{
            if(!String(file.name??'').toLowerCase().endsWith('.bin'))return {energy:null,machine:null,nominalDoseRate:null};
            return inspectBinHeader(await (file.slice?file.slice(0,1024):file).arrayBuffer());
        }));
        const energies=[...new Set(rows.map(r=>r.energy).filter(Boolean))],machines=[...new Set(rows.map(r=>r.machine).filter(Boolean))];
        if(energies.length>1)throw new Error('Mixed energies detected. Calibrate each energy separately.');
        if(machines.length>1)throw new Error('Mixed RDS/TDS logs detected. Calibrate each machine separately.');
        const complete=rows.length>0&&rows.every(r=>r.energy&&r.nominalDoseRate);
        return {energy:complete?energies[0]:null,machine:machines[0]??null,nominalDoseRate:complete?rows[0].nominalDoseRate:null};
    }
    function parseBin(input, options={}) {
        ...
        if(mlcModel===6 && leafCount!==114)throw new Error('Invalid RDS model 6 MLC layout; expected 114 leaves plus two carriage channels.');
        // Keep numeric subbeam metadata only; skip names and identifiers.
        // inspectBinHeader separately extracts only the energy metadata.
        ...
        for(let i=0;i<snapshots;i++) {
            ...
            for(const which of ['expected','actual']) {
                ...
                if(options.playback && axes.has(9)) { const raw=view.getFloat32(base+axes.get(9).offset+side,true); target.couch=mlcModel===6 && raw>1e30?null:read(axes.get(9).offset+side); }
                target.jaws=mlcModel===6?null:jaws.map(offset=>read(offset+side)*10);
        ...
        return {metadata:{...inspectBinHeader(bytes),samplingIntervalMs,subbeams:Math.max(1,subbeams),axisScale,truncated,snapshots,version,mlcModel,format:'bin'},samples,leafCount,subbeamHeaders};
    }
    ...
    function deliveryWindows(log) {
        ...
        // BIN subbeam CP markers distinguish internal holds from new arcs.
        const headers=log.subbeamHeaders;
        if(runs.length!==log.metadata.subbeams && headers?.length===log.metadata.subbeams && headers.length>1 && headers.every((h,i)=>!i||h.cp>headers[i-1].cp)) {
            const groups=headers.map(()=>[]);
            for(const run of runs){
                let group=0;while(group+1<headers.length && s[run.start].actual.cp>=headers[group+1].cp-.05)group++;
                groups[group].push(run);
            }
            if(groups.every((g,i)=>g.length && Math.abs(Math.round(s[g[0].start].actual.cp)-headers[i].cp)<=1 && (!headers[i+1]||s[g.at(-1).end].actual.cp<=headers[i+1].cp+1.05))) {
                runs.splice(0,runs.length,...groups.map(g=>({start:g[0].start,end:g.at(-1).end})));
            }
        }
    ...
    function interpolate(log, cp, window) {
        ...
        return {time:lerp(left.time,right.time)-s[window.start].time,mu:lerp(left.expected.mu,right.expected.mu),gantry:angle(left.expected.gantry,right.expected.gantry),collimator:angle(left.expected.collimator,right.expected.collimator),leaves:left.expected.leaves.map((x,i)=>lerp(x,right.expected.leaves[i])),jaws:left.expected.jaws?left.expected.jaws.map((x,i)=>lerp(x,right.expected.jaws[i])):null};
    }
    function reconstruct(log, nominalDoseRate=600) {
        ...
        return deliveryWindows(log).map((w,number)=>{
            ...
            const cps=points.map((p,i)=>({controlPointIndex:i,gantryAngle:p.gantry,collimatorAngle:p.collimator,doseRateSet:nominalDoseRate,cumulativeMetersetWeight:(p.mu-first)/(last-first),mlcPositionData:log.metadata.mlcModel===6?mlcLayers(p.leaves,6,log.metadata.axisScale):[{type:'MLCX',positions:p.leaves}],asymx:p.jaws?.slice(0,2)??null,asymy:p.jaws?.slice(2)??null}));
            return {beam:{beamNumber:number+1,beamName:`Log subbeam ${number+1}`,totalMeterset:last-first,controlPoints:cps,isRDSMachine:log.metadata.mlcModel===6,mlcDefinitions:log.metadata.mlcModel===6?mlcGeometry(6,log.leafCount):[],finalCumulativeMetersetWeight:1},measuredSeconds:w.seconds,measuredCpTimes:points.map(p=>p.time),window:w,source:'log-commanded-control-points'};
        });
    }
    ...
    function validateProfile(profile, limits=null) {
        ...
        if(profile.model===LOCAL_MODEL){
            ...
            if(profile.parameters?.rdsTransitionModel!==undefined && (profile.parameters.rdsTransitionModel!=='planned-zero-mu-v1'||profile.calibration?.machine!=='RDS'))throw new Error('Unsupported RDS transition model.');
            const maximum=profile.calibration?.machine==='RDS'?1000:180;
            if(!Number.isFinite(response)||response<20||response>maximum)throw new Error(`Invalid local leaf-response parameter (20–${maximum} mm/s²).`);
        }else{
    ...
    function validateBeam(profile,beam){
        if(profile?.model===LOCAL_MODEL){
            const machine=profile.calibration?.machine || 'TDS';
            if((machine==='RDS')!==!!beam.isRDSMachine)throw new Error(`This profile is fitted for ${machine}; use a matching machine profile.`);
            axisResponse().prepare(beam,profile.nominalLimits);
        }
    }
    ...
    function description(profile){
        return profile.model===LOCAL_MODEL?`Local axis response${profile.parameters.rdsTransitionModel?' + planned transitions':''} (${profile.parameters.leafResponseAcceleration.toFixed(0)} mm/s² response parameter)`:`${profile.parameters.timeScale.toFixed(4)}× baseline beam time`;
    }
    function report(profile,files,outside){
        ...
        if(local){
            ...
            if(c.parameterAtSearchBoundary)text+='\nFit reached the parameter search boundary; review held-out errors before applying.';
            if(profile.parameters.rdsTransitionModel)text+='\nRDS planned zero-MU transitions include acceleration-limited approach and restart. Unplanned pauses are not predicted.';
    ...
    const api={mlcGeometry,mlcLayers,inspectBinHeader,inspectFiles,MODEL,LOCAL_MODEL,description,report,fitForMethod,validateBeam,DEFAULT_LIMITS,parseCsv,parseBin,readFile,deliveryWindows,reconstruct,apply,fit,validateProfile,comparePlan,diagnostics,quantile};
