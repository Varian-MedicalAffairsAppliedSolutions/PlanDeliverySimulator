...
function prepare(beam,limits){
    ...
    const rows=cps.slice(1).map((cp,i)=>{
        ...
        const layerCount=beam.isRDSMachine?2:1;
        if(a?.length!==layerCount||b?.length!==layerCount)throw new Error('Local timing requires consistent single-layer or RDS dual-layer MLC positions.');
        const types=beam.isRDSMachine?['MLCX1','MLCX2']:[a[0].type];
        if(beam.isRDSMachine && a.map(l=>l.positions.length).sort((x,y)=>x-y).join(',')!=='56,58')throw new Error('RDS requires 28 and 29 leaf pairs.');
        const leaves=types.flatMap(type=>{
            const before=a.find(l=>l.type===type)?.positions,after=b.find(l=>l.type===type)?.positions;
            if(!before?.length||!after||before.length!==after.length)throw new Error('Inconsistent MLC layer layout.');
            return after.map((v,j)=>v-before[j]);
        });
...
// Empirical synchronized-path envelope for RDS start/end and planned beam-off
// transitions. Derive stops from MU weights, never from measured hold timings.
function transitionDurations(prepared,response){
    const result=durations(prepared,response),r=prepared.rows,l=prepared.limits,n=r.length;
    if(!Number.isFinite(l.maxMlcAccelDecelInput)||l.maxMlcAccelDecelInput<=0)throw new Error('Invalid MLC transition acceleration.');
    const distance=r.map(x=>Math.max(1e-8,Math.abs(x.g)/l.maxGantrySpeedInput,x.leaf/(10*l.maxMlcSpeedInput),Math.abs(x.c)/l.maxCollimatorSpeedInput));
    const caps=distance.map((d,i)=>d/result.times[i]);
    const acceleration=r.map((x,i)=>Math.min(...[[Math.abs(x.g),l.maxGantryAccelDecelInput],[x.leaf,10*l.maxMlcAccelDecelInput],[Math.abs(x.c),l.maxCollimatorAccelDecelInput]]
        .filter(([d])=>d>1e-8).map(([d,a])=>a*distance[i]/d),Infinity));
    const speeds=Array.from({length:n+1},(_,i)=>i===0||i===n?0:Math.min(caps[i-1],caps[i]));
    // Float32 MU and interpolation can leave a tiny residue in a zero-MU span.
    const tolerance=Math.max(1e-4,prepared.beam.totalMeterset*Math.pow(2,-22));
    for(let i=1;i<n;i++)if((r[i-1].mu<=tolerance)!==(r[i].mu<=tolerance))speeds[i]=0;
    for(let i=0;i<n;i++)speeds[i+1]=Math.min(speeds[i+1],Math.sqrt(speeds[i]**2+2*acceleration[i]*distance[i]));
    for(let i=n-1;i>=0;i--)speeds[i]=Math.min(speeds[i],Math.sqrt(speeds[i+1]**2+2*acceleration[i]*distance[i]));
    const motion=distance.map((d,i)=>{
        const a=acceleration[i],start=speeds[i],end=speeds[i+1];
        if(!Number.isFinite(a))return null;
        const peak=Math.min(caps[i],Math.sqrt(a*d+(start*start+end*end)/2));
        const up=(peak-start)/a,down=(peak-end)/a;
        const cruise=Math.max(0,d-(2*peak*peak-start*start-end*end)/(2*a))/peak;
        const seconds=up+cruise+down;
        if(seconds>result.times[i]+1e-8)result.causes[i]='Planned transition response';
        result.times[i]=seconds;
        return {distance:d,acceleration:a,start,peak,end,up,cruise,down};
    });
    return {...result,motion};
}
function motionAt(m,elapsed){
    const t=Math.max(0,Math.min(m.up+m.cruise+m.down,elapsed));
    let position,speed;
    if(t<=m.up){speed=m.start+m.acceleration*t;position=m.start*t+m.acceleration*t*t/2;}
    else if(t<=m.up+m.cruise){speed=m.peak;position=(m.start+m.peak)*m.up/2+m.peak*(t-m.up);}
    else {const tail=t-m.up-m.cruise;speed=m.peak-m.acceleration*tail;position=(m.start+m.peak)*m.up/2+m.peak*m.cruise+m.peak*tail-m.acceleration*tail*tail/2;}
    return {fraction:Math.max(0,Math.min(1,position/m.distance)),rate:Math.max(0,speed/m.distance)};
}
function modelDurations(prepared,parameters){
    if(parameters.rdsTransitionModel!==undefined && (parameters.rdsTransitionModel!=='planned-zero-mu-v1'||!prepared.beam.isRDSMachine))throw new Error('Unsupported RDS transition model.');
    return parameters.rdsTransitionModel?transitionDurations(prepared,parameters.leafResponseAcceleration):durations(prepared,parameters.leafResponseAcceleration);
}
function predict(beam,limits,parameters){
    if(!Number.isFinite(parameters?.leafResponseAcceleration)||parameters.leafResponseAcceleration<20||parameters.leafResponseAcceleration>(beam.isRDSMachine?1000:180))throw new Error('Invalid local leaf-response parameter.');
    const p=prepare(beam,limits),{times,causes,motion}=modelDurations(p,parameters);let total=0;
    const data=p.rows.map((r,i)=>{
        const duration=times[i],row={cp:r.cp,cumulativeSimTime:total,segmentDuration:duration,
            ...
            limitingComponent:causes[i],componentTimes:r.components,...(motion?.[i]?{motionProfile:motion[i]}:{}),
...
function fit(pairs,limits,label='Local axis-response calibration'){
    ...
    if(new Set(pairs.map(p=>!!p.beam.isRDSMachine)).size>1)throw new Error('Fit RDS and TDS deliveries separately.');
    ...
    const candidates=pairs[0].beam.isRDSMachine?Array.from({length:197},(_,i)=>20+5*i):Array.from({length:161},(_,i)=>20+i);
    const transition=pairs[0].beam.isRDSMachine?{rdsTransitionModel:'planned-zero-mu-v1'}:{};
    const predictions=candidates.map(a=>prepared.map(p=>modelDurations(p,{leafResponseAcceleration:a,...transition}).times));
    ...
    return {schema:'delivery-timing-profile',version:1,model:MODEL,label:String(label).slice(0,120),nominalLimits:{...limits},
        parameters:{leafResponseAcceleration:candidates[best],...transition},
        calibration:{machine:pairs[0].beam.isRDSMachine?'RDS':'TDS',method:'Local coupled axis response',source:'log-commanded-control-points',deliveryCount:pairs.length,targetWindowSeconds:1,
            ...
            parameterAtSearchBoundary:best===0||best===candidates.length-1,
...
const api={MODEL,prepare,durations,transitionDurations,motionAt,predict,windows,evaluate,fit};
