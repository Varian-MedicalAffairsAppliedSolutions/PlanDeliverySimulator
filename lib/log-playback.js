...
function buildReplay(log){
    const calibration=typeof module!=='undefined'&&module.exports?require('./timing-calibration.js'):root.TimingCalibration;
    const definitions=calibration.mlcGeometry(log.metadata.mlcModel,log.leafCount);
    ...
    const data=log.samples.map((sample,i)=>{
        ...
        const cp={controlPointIndex:i,gantryAngle:angle(a.gantry),collimatorAngle:angle(a.collimator),
            ...
            mlcPositionData:calibration.mlcLayers(a.leaves,log.metadata.mlcModel,log.metadata.axisScale),
            asymx:a.jaws?[native?-a.jaws[0]:a.jaws[0],a.jaws[1]]:null,asymy:a.jaws?[native?-a.jaws[2]:a.jaws[2],a.jaws[3]]:null};
    ...
    const beam={beamNumber:1,beamName:'Recorded BIN',isRDSMachine:log.metadata.mlcModel===6,mlcDefinitions:definitions,controlPoints:data.map(r=>r.cp),totalTime:data.at(-1).cumulativeSimTime};
